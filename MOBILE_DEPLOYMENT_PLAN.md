...
1. **You:** start Apple Developer Program + Google Play Console enrollment now (Phase 0.5) — this has an external waiting period, so kicking it off early avoids it becoming the critical path later. Also create the OneSignal account/app.
2. **Me (when you're ready to start building):** begin Phase 0 — migrate the Gemini API key behind a Cloud Function. This is self-contained, improves the current live web app immediately, and doesn't depend on any store account being ready.
