...
- Verified with `tsc --noEmit` and `npm run build` (both clean), committed, pushed, and deployed to `signallinglogbook.com` via `npx vercel --prod --scope matt-ward1` — confirmed live (`HTTP 200`). **Not yet end-to-end tested** with a real fresh link in an incognito/logged-out session — recommended next step before considering this fully closed.
...
Last **production** deploy is commit `62aaaf2` (blueprint doc — no code change since `bdf84cb`, which is the last commit that actually touched shipped app code). The PDF customiser badge fix and the supervisor verification portal `permission-denied` fix are both **shipped and live** as of this session, on top of everything from prior sessions (certifier declaration + Quick Parts, both Web sign-in blockers, print-layout fixes). Open items, roughly in priority order:

1. **You (or me together):** end-to-end test the supervisor verification portal fix with a *freshly generated* link in an incognito/logged-out browser — this was fixed and deployed this session but not yet actually re-tested live. Recommended before considering it fully closed. Remember any pre-existing pending verification requests need re-issuing (old-format tokens no longer resolve).
2. **You:** fill in and return `Default_Config_Template.xlsx` (repo root) so I can update the app's default dropdown lists.
3. **You:** add Android/iOS apps in the Firebase Console + place `google-services.json` / `GoogleService-Info.plist` (Phase 2, §6) so native Google Sign-In can eventually be tested.
4. **You:** resolve the Apple Developer Program enrollment error at `developer.apple.com/account`, and keep progressing the Google Play Console app listing.
5. **Me, next engineering:** **Phase 4 (push notifications / OneSignal)** is fully unblocked and can start any time — it will eventually need a OneSignal account (not urgent until the certificate-upload step) and, for real end-to-end testing, a Cloud Functions deploy + device access.
6. Longer-term: Phase 7/8 (device testing, store submission) are blocked on Android Studio/SDK and a Mac+Xcode, neither present on this machine.
...
- **Supervisor verification portal `permission-denied` bug fixed:** anonymous lookup was silently guaranteed to fail for every real (logged-out) supervisor due to a Firestore `list`-vs-`get` rules mismatch — switched the token to the log's own document id and the lookup to a direct `getDoc`. Committed `bdf84cb`, deployed. Needs a fresh end-to-end retest (see item 1 above).
