...
> Companion docs: `PROJECT_OVERVIEW.md` (deep-dive on the existing web app's architecture), `MOBILE_DEPLOYMENT_PLAN.md` (original detailed mobile plan + phase-by-phase progress log), and `PROTOTYPE_TO_PRODUCTION_BLUEPRINT.md` (a generalized, stack-agnostic runbook distilled from this project's setup journey — intended to be copied into *other* new prototype projects, not specific to this one). This file consolidates the first two into one current-state summary.
...
- **Firebase CLI now set up (earlier session):** plain interactive `firebase login` crashes on this machine with a native libuv assertion (`Assertion failed: !(handle->flags & UV_HANDLE_CLOSING), file src\win\async.c`) — a Windows-specific CLI bug, not a project issue. Workaround that works reliably: `firebase login --no-localhost`, then open the printed `https://auth.firebase.tools/login?...` URL (used the in-IDE browser tool to do this hands-free) and paste the resulting authorization code back into the CLI prompt. Logged in as `mgward86@gmail.com`. Added `.firebaserc` (`default` project → `gen-lang-client-0452980140`) and `firebase.json` (multi-database array form, since this project's Firestore lives on a **named** database — `ai-studio-3660dd77-6f01-4cee-9cd4-5a3fa86b3647`, not `(default)`) so `firebase deploy --only firestore:<databaseId>` targets the right database. Both files were committed in `bb61bb1`.

### Environment reset & full restore (this session)
This machine's dev environment was reset again since the last session — `git`, `node`, and `npm` were completely absent (not just off `PATH`; not installed at all), and every CLI credential (git identity, GitHub push auth, Vercel login, Firebase login) was gone, even though the project folder itself (`.git`, `node_modules`, config files) had survived intact. Full restore performed and verified end-to-end:
1. **Git 2.55.0** + **Node.js 24.19.0 LTS** reinstalled via `winget`. Hit the same known gotcha as before: Cursor (and its spawned terminals) kept the stale pre-install environment snapshot until it was **fully quit and relaunched** (not "Reload Window") — confirmed working again after that.
2. `npm install` + `npx tsc --noEmit` + `npm run build` all verified clean on the restored toolchain.
3. Git identity reset to match existing commit history (`mgward86 <mgward86@users.noreply.github.com>`); GitHub reconnected via Cursor's `ConnectScm`; push/pull access re-verified with `git fetch` + `git push --dry-run` (this hung for several minutes waiting on a Git Credential Manager browser sign-in prompt before completing on its own — not a bug, just needed user interaction).
4. **Vercel CLI** re-authenticated via `npx vercel login` (device-authorization flow — an in-IDE-browser attempt failed since that browser had no saved session; completed manually in the user's own browser). First attempt errored with a generic `Error: fetch failed` after a long poll — retried and succeeded on the second device code. Logged in as `mgward86-8077`; deploys still need `--scope matt-ward1` since the project lives under that team.
5. **Firebase CLI** re-authenticated via the same `--no-localhost` workaround as before. First login link went stale (left unused too long → `{"error":"invalid_request","error_description":"Unable to verify client."}`) — regenerated a fresh one and completed it promptly. The login (and the follow-up `firebase login <code>`) both still crash-on-exit with the same libuv assertion, but only *after* printing a real success message — confirmed the token actually persisted via `firebase login:list` / `firebase projects:list` despite the crash.
6. Wrote **`PROTOTYPE_TO_PRODUCTION_BLUEPRINT.md`** (commit `62aaaf2`) — a generalized version of this whole restore-and-setup journey (plus the earlier Capacitor/mobile-wrap phases), meant to be copied into future new prototype projects as a standing runbook, not maintained as part of this app's own status tracking.
...
### ✅ "Cancel Approval Request" button reappearing after cancellation (this session)
- You reported: after submitting then cancelling a verification request, reopening "Request Supervisor Verification" correctly shows the "Previous Approval Request Cancelled" banner, but the **"Cancel Approval Request" button was still visible** — nothing left to cancel.
- Root cause: `SupervisorVerificationModal.tsx`'s local `generatedToken` state is initialized from `log.verificationToken`, which is **intentionally never cleared** on cancellation (it's the log's own Firestore doc id, doubling as the security-rule access key so a cancelled portal link still resolves to a friendly "cancelled" screen instead of a permission error). The button-visibility check was `log.verificationStatus === 'pending_verification' || generatedToken` — since `generatedToken` stays truthy forever once a request has ever been made, the cancel button kept showing on every reopen regardless of actual status (cancelled, verified, rejected).
- Fix: added a dedicated `hasActiveRequest` state, initialized only from `log.verificationStatus === 'pending_verification'`, flipped `true` right after a successful generate and `false` right after a successful cancel. Both "Cancel Approval Request" button conditions (Configure tab and Share tab) now key off `hasActiveRequest` instead of the token's mere existence. `generatedToken` is unchanged and still solely controls the share-link URL/tab-enablement, which was working correctly already.
...
### ✅ PDF customiser preview/generator drift fix (commit `0937fd4`, deployed)
- You reported the "Customise PDF Export Template" modal's **Quick Preview** showed a green "Certified & Recorded" badge next to the "LOG DETAILS & PERSONNEL RECORD" header (Executive Modern style) that never appeared in the actual generated PDF for that style.
- Root cause: `pdfGenerator.ts`'s `generateLogPage()` branches per header style — **Executive Modern** and **Condensed Table** each have their own fully self-contained rendering path and never reach the shared "Unified Log Details & Personnel Record" table (used only by Solid Banner / Bold Left Border / Accent Lines), which is the *only* place that badge (`STATUS: Certified & Recorded`) was ever drawn. The Quick Preview's approximation, however, showed that badge unconditionally under **all four** non-Condensed header styles, including Executive Modern — a genuine preview/generator drift, the exact class of bug the shared `pdfGenerator.ts` extraction (see PDF customiser UX overhaul, below) was meant to prevent, just in a spot that overhaul hadn't touched.
- Fix: removed the badge from **both** places rather than special-casing per style — it's gone from the shared details table in `pdfGenerator.ts` (the `Name:` cell now spans the freed width) and from the Quick Preview markup in `UserProfileForm.tsx`. Net effect: it's now consistently absent everywhere, for every header style, in both the preview and the real PDF.
- Flagged (not fixed, out of scope for this fix): Executive Modern's real PDF body layout is a distinct 3-column grid under a `LOG ENTRY BY <name — role, RIW>` header, structurally different from the generic card the Quick Preview approximates for it. That's a deeper, pre-existing drift than just this badge — the modal's own copy already discloses the Quick Preview is "a fast, approximate sketch," and "Preview Real PDF" (which calls the exact real code) remains the authoritative check. Not tackled since it's a much larger rebuild than what was asked; flagged for a future pass if wanted.
- Verified with `tsc --noEmit` and `npm run build` (both clean), committed, pushed, and deployed to `signallinglogbook.com` via `npx vercel --prod --scope matt-ward1` — confirmed live (`HTTP 200`).

### ✅ Supervisor verification portal `permission-denied` bug fix (commit `bdf84cb`, deployed)
- You tested the supervisor sign-off link flow and hit **"Error retrieving log entry. Please verify connection."** when looking up a verification code.
- Root cause: `SupervisorPortalModal.tsx`'s lookup ran a **collection query** (`query(collection(db,'logEntries'), where('verificationToken','==',token))`) — a Firestore `list` operation. `firestore.rules`'s `list` rule for `logEntries` correctly requires `isSignedIn() && resource.data.userId == request.auth.uid`, which an anonymous supervisor clicking an emailed link can never satisfy — so **every** real anonymous lookup was being denied with `permission-denied`, not a connectivity issue. The rules file *does* already have a correctly-scoped exception for a single-document `get` (`resource.data.verificationToken != null && resource.data.verificationToken != ''`), but the client never actually performed a `get`-by-id, so that exception was unreachable. This was likely never caught before because internal testing was probably done while signed in as the technician's own account (which incidentally satisfies the owner-based `list` rule too), rather than from a genuinely logged-out session.
- **Deliberately did not** just widen the `list` rule to allow this — doing so would let *any* anonymous caller enumerate every pending/verified log entry in the database (Firestore can't verify a `list` query's filter value matches a specific known secret; it can only check `resource.data` per already-matched document, which doesn't require the caller to have known that secret up front). A `get`-by-id is safe specifically because the caller must already know the exact, unguessable Firestore document id.
- Fix: `SupervisorVerificationModal.tsx` now sets the verification token to the log's own Firestore document id (`log.id`) instead of a separately-generated `verif_<timestamp>_<random>` string; `SupervisorPortalModal.tsx`'s lookup now does a direct `getDoc(doc(db,'logEntries', token))`. No `firestore.rules` changes needed or deployed — the existing `get` rule already covered this correctly once the client used the right operation.
- **Important caveat, not yet actioned:** any verification links generated *before* this fix carry the old token format and will no longer resolve — there's no data migration needed, but any currently in-flight supervisor requests need the technician to re-open "Request Verification" and re-issue to get a working link.
- Verified with `tsc --noEmit` and `npm run build` (both clean), committed, pushed, and deployed to `signallinglogbook.com` via `npx vercel --prod --scope matt-ward1` — confirmed live (`HTTP 200`). **✅ End-to-end retested by the user with a fresh link in an incognito/logged-out session — confirmed working.** This item is now fully closed.
...
Last **production** deploy is commit `62aaaf2` (blueprint doc — no code change since `bdf84cb`, which is the last commit that actually touched shipped app code). The PDF customiser badge fix and the supervisor verification portal `permission-denied` fix are both **shipped, live, and now fully verified** as of this session, on top of everything from prior sessions (certifier declaration + Quick Parts, both Web sign-in blockers, print-layout fixes). Open items, roughly in priority order:
...
4. **Me, next engineering:** **Phase 4 (push notifications / OneSignal)** is fully unblocked and can start any time — it will eventually need a OneSignal account (not urgent until the certificate-upload step) and, for real end-to-end testing, a Cloud Functions deploy + device access.
...
- **Local dev environment fully restored from scratch:** Git + Node.js reinstalled, all CLI logins (GitHub push access, Vercel, Firebase) re-established and verified — see §5. New reusable `PROTOTYPE_TO_PRODUCTION_BLUEPRINT.md` written to generalize this restore process (and the earlier Capacitor/mobile phases) for future new projects.
- **PDF customiser preview/generator drift fixed:** removed a "Certified & Recorded" badge that showed in the Quick Preview for every header style but was only ever real in the generated PDF for three of the five styles — now consistently absent everywhere. Committed `0937fd4`, deployed.
- **Supervisor verification portal `permission-denied` bug fixed and verified:** anonymous lookup was silently guaranteed to fail for every real (logged-out) supervisor due to a Firestore `list`-vs-`get` rules mismatch — switched the token to the log's own document id and the lookup to a direct `getDoc`. Committed `bdf84cb`, deployed, and **confirmed working end-to-end by the user with a fresh link in an incognito/logged-out session.** Fully closed — no further action needed.
